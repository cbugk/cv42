...
* You like to build a CV optimized for 2 pages incl. a sidebanner (with option for
  additional pages)
...
* all valid options for the standard article class like e.g. `a4paper` or
  `11pt`.
* `sidesectionsize` sets the font size of sidebanner section titles to `\large`,
  `\huge`, etc. when assigned to `large`, `huge`, etc. (i.e. w/o leading slash)
* `showframes` will show frames around the sidebanner and body texts that help to
...
* `vline=<length>` plots a red vertical line at x-position `<length>` to help
  adjust sidebanner content.
* `maincolor=<color>` sets the theme color used for the entire sidebanner incl.
  headings, icons, and charts. Many colors are derived from it.
* `sidecolor=<color>` sets the background color of the sidebanner.
* `sidetextcolor=<color>` sets the color of text located in the sidebanner.
...
* `sidebannerwidth=<length>` sets the total width of the sidebanner, i.e. the width
  of the visible sidebanner color.
...
* `sidebannerplacement=right` puts the sidebanner to the right of the body instead of
  to the left.
...
sidebanners on front and back of the CV are generated using the commands
`\makefrontsidebanner` and `\makebacksidebanner` within the `document` part of the
LaTeX code. It is recommended to use `\newpage` immediately before
`\makebacksidebanner`.

Layout elements in front and back sidebanner can be added either by using
```latex
...
```latex
\renewcommand{\makefrontsidebanner}{%
  \begin{sidebanner}
    <your code>
  \end{sidebanner}
}
...
The `sidebanner` environment makes sure that at least all text will be positioned
correctly within the left column (sidebanner) of each page. If you need something
more restrictive, e.g. to prevent overflow when using multiple chart labels,
you can enclose parts of the sidebanner content in a `sidebannerminipage`:
```latex
...
```
Using the `sidebanner` environment, you can also add more sidebanner definitions for
additional pages easily. If you want the sidebanner to appear on the right instead
of the left side of the body, use the class option `sidebannerplacement=right`.

If you want to remove the sidebanner entirely on a single page, just don't call
any of the sidebanner constructors. Then you may set your preferred margins using
```latex
\newpage
% no \makebacksidebanner etc.
\newgeometry{
  top=<length>,
  bottom=<length>,
  left=<length>,
...
The text color of sidebanner elements can be adapted via the class option
...
### Further sidebanner Style Elements
...
* When working on multi-language documents, I highly advise against the usage
  ...
  `sidebannerplacement=right` feature as well.
...
* If you want to change the base font size, use the 'article' class options
  like `12pt` for all body elements and the paragraphs in the sidebanner. sidebanner
  ...
  redefine the respective commands as explained [here](#sidebanner).
